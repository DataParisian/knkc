...
*		 number, part name, and quantity on hand. Parts the parts          *
*		 sorted by part number.     	   *
****************************************************************************/
void print(void){
	int i, j;
	int part_nums[MAX_PARTS] = {0};	
	
	for (i = 0; i < num_parts; i++)
		part_nums[i] = inventory[i].number;
	
	quicksort (part_nums, 0, num_parts - 1);
		
	printf("Part number		Part Name			Quantity on Hand\n");
	
	for (i = 0; i < num_parts; i++)
		for (j = 0; j < num_parts; j++){
			if (part_nums[i] == inventory[j].number){
				printf("%7d			%-25s%11d\n", inventory[j].number,
